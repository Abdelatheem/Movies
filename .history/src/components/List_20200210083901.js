...
class Item extends PureComponent {
    render() {
        return (
            <View key={this.props.item.item.id} style={{ width: 130 }}>
                <TouchableOpacity onPress={() => { this.props.moveDetails(this.props.item.item.id) }}  disabled={isDisabled}>
                    <Image
                    style={{ width: 130, height: 200, borderRadius: 5 }}
                    source={{ uri: `https://image.tmdb.org/t/p/w200${this.props.item.item.poster_path}` }}
                    resizeMode="stretch"
                    />
                    ...
                    <Text style={{color: '#00d374', textAlign: 'center', fontSize: 12}}>
                        {this.props.item.item.release_date ? this.props.item.item.release_date.split('-')[0] : ''}
                    </Text>
                    <View style={{flexDirection: 'row'}}>
                        <Icon name={'star'} iconStyle={{margin: 3}} type={'antdesign'} size={15} color="#f0c40e"/>
                        <Text style={{color: '#FFF', textAlign: 'center', fontSize: 15}}>{this.props.item.item.vote_average}</Text>
                    </View>
                    </View>
                </TouchableOpacity>  
            </View>
...
export class List extends Component  {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (this.props.generMovies !== nextProps.generMovies) {
          return true;
        }
        return false;
      }
    ...
    renderItem(item){
        return <Item item={item} moveDetails={this.props.moveDetails} />
    }
