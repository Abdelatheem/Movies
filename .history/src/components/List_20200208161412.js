...
export class List extends Component {
    ...
    async renderMovies(id){
        if(id){
            await axios.get(BASE_URL + '/discover/movie?' + API_KEY + '&sort_by=popularity.desc&with_genres='+id)
            .then(result => {
                var result = Object.values(result.data)[3];
                generMovies2.push(result)
            })
            .catch((error) => {
                console.log(error.response);
            });
            this.sorting(generMovies2);
        }
    }
