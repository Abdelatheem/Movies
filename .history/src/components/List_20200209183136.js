...
export class List extends PureComponent  {
    ...
    renderMovies(id){
        setTimeout(function() { this.setState({inClick: false}); }, 2000);
        if(id){
            console.log(id);
    ...
    render() {
        return (
            <FlatList
                ...
                <TouchableOpacity onPress={() => { !this.state.inClick ? this.props.moveDetails(item.id, item.poster_path) : null }}  disabled={isDisabled}>
                <Image
