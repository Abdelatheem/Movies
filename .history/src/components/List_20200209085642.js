    ...
    render() {
        return (
            <FlatList
                ...
                horizontal
                renderItem={({ item }) => <View key={item.id} style={{ width: 130, borderRadus: 10, borderColor: 'rgba(255,255,255,0.2)', borderWidth: 1 }}>
                <TouchableOpacity onPress={() => this.props.moveDetails(item.id, item.poster_path)}>
                <Image
