...
class MovieDetail extends Component {
    ...
    renderDetails(){
        ...
        if(this.props.movieDetails){
            const {poster_path, original_title, vote_count, vote_average, tagline } = this.props.movieDetails
            return  <View>
                    <Text style={styles.movieTitle}>{original_title}</Text>
                    <Text style={styles.movieTitle2}>{tagline}</Text>
                    <Image source={{ uri: `https://image.tmdb.org/t/p/w342${poster_path}` }} style={{width: '50%', height: 300,  resizeMode: 'contain' }} />
                    <Card>                    
