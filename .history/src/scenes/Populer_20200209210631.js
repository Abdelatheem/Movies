...
class Populer extends Component {
  ...
  renderResults(){
    if(this.props.moviesResults){
      ...
      this.props.moviesResults.map(movie => {
        var oneMovie = {id: movie.id, title: movie.title, poster_path: movie.poster_path, release_date: movie.release_date, vote_average: movie.vote_average}; 
      ...
      return <FlatList
          ...
          onEndReached={() => { counter++; this.props.getPopulerMovies(counter)}}
