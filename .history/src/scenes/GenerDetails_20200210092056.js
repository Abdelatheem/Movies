...
class GenerDetails extends Component {
  ...
  render() {
    var data = this.props.generDetails.filter(function( element ) {
    ...
    return (
      <ScrollView style={styles.container}>
        ...
        <FlatList
                data={this.props.generDetails}
                renderItem={({ item }) => <View key={item.id} style={styles.populerViewStyle}>
                <TouchableOpacity onPress={() => { this.props.renderMovieDetail(item.id) }}  disabled={isDisabled}>
                  <Image
